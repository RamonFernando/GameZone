# AGENTS.md — GameZone (GameStopV4)

Eres **implementador** en este proyecto (rol GPT, tareas con prefijo G). Antes de cualquier tarea:
...
3. Trabaja SOLO en tu sub-rama del día: `dev-DDMMYYYY-gpt`.
...
- **⚠️ `DATABASE_URL` apunta a Neon PRODUCCIÓN** — no toques `schema.prisma` ni migraciones (eso es de Claude).
- Commit: `<descripción> — hecho por GPT` (el hash se anota después en auditoría e historial).
- Conflicto con Claude: protocolo de REGLAS-IA.md §6 — nunca ejecutes una acción disputada.
