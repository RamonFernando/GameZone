...
const ITEMS = [{ slug: "sonic-frontiers", quantity: 1 }];

function fail(message, details) {
  console.error(`\n[E2E PAYPAL] ERROR: ${message}`);
  if (details) {
...
async function run() {
  console.log(`[E2E PAYPAL] Base URL: ${BASE_URL}`);
  console.log(`[E2E PAYPAL] Login con: ${IDENTIFIER}`);
  ...
  const checkoutResult = await requestJson(`${BASE_URL}/api/payments/paypal/create-order`, {
    method: "POST",
    headers: {
  ...
  if (!checkoutResult.response.ok) {
    fail("No se pudo crear orden de PayPal.", checkoutResult.payload);
  }

  const checkoutUrl = String(checkoutResult.payload?.checkoutUrl ?? "");
  if (!checkoutUrl || !checkoutUrl.includes("paypal")) {
    fail("Respuesta sin checkoutUrl valido.", checkoutResult.payload);
  }

  console.log("[E2E PAYPAL] Orden creada correctamente.");
  console.log(`[E2E PAYPAL] URL: ${checkoutUrl}`);
  console.log("[E2E PAYPAL] OK.");
}
